from bias_variance import plot_erbiva_lda, er_bi_va_bootstrap_Lasso
...
for i, poly_degree in enumerate(tqdm(degree_list)):
    ...
    gridsearch = GridSearchCV(estimator=Lasso(), param_grid=dict(alpha=lda_ray))
    ...
    error_ray[i], bias_ray[i], variance_ray[i] = er_bi_va_bootstrap_Lasso(
...
plt.ylabel("[]")
plt.title(f"bias-variance tradeoff, lasso with optimal $\\lambda$, ${n}^2$ datapoints")
plt.savefig("imgs/franke/bias-variance_lasso.png", dpi=200)
plt.clf()

plot_erbiva_lda(7, x_grid, y_grid, z_grid, lda_ray, regtype="lasso")
plot_erbiva_lda(13, x_grid, y_grid, z_grid, lda_ray, regtype="lasso")
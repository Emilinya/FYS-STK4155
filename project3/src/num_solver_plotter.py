...
def get_data():
    files = [v[2] for v in os.walk("data/num_solver")][0]
    dxs = sorted([
    ...
    return {
        dx:
        {
            "t_ray": np.load(f"data/num_solver/t_ray_dx={dx}.npy"),
            "ana_u_grid": np.load(f"data/num_solver/ana_u_grid_dx={dx}.npy"),
            "num_u_grid": np.load(f"data/num_solver/num_u_grid_dx={dx}.npy"),
        } for dx in dxs}
...
def main():
    ...
    for f in [0, np.log(2)/np.log(100), 1]:
        ...
        plt.xlabel("x []")
        plt.ylabel("u []")
        plt.savefig(f"imgs/num_solver_imgs/{t=:.3g}_comp.svg")
        plt.clf()
        ...
        plt.xlabel("x []")
        plt.ylabel("rel err [%]")
        plt.savefig(f"imgs/num_solver_imgs/{t=:.3g}_rel_err.svg")

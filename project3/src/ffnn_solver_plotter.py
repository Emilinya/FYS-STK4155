...
data = np.load(f"data/ffnn_solver/hn=10_tp=SIGMOID.npz")
...
plt.savefig(f"imgs/ffnn_solver/loss_grid.svg")
plt.clf()
...
for f in [0, np.log(2)/np.log(100), 1]:
    ...
    plt.xlabel("x []")
    plt.ylabel("u []")
    plt.savefig(f"imgs/ffnn_solver/t={t_ray[idx]:.3g}comp.svg")
    plt.clf()
...
plt.xlabel("t []")
plt.ylabel("abs_diff []")
plt.savefig(f"imgs/ffnn_solver/abs_diff.svg")
plt.clf()
...
plt.xlabel("t []")
plt.ylabel("rel_diff []")
plt.savefig(f"imgs/ffnn_solver/rel_diff.svg")
plt.clf()
